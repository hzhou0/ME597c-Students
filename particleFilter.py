...
class particleFilter(Node):
    def __init__(self, mapFilename="your_map/room.yaml", numParticles=500):
        ...
        qos_profile_laserScanner = QoSProfile(
            reliability=ReliabilityPolicy.RELIABLE,
            durability=DurabilityPolicy.VOLATILE,
            depth=10,
        )
        # Create the odom and laserScan subscribers with ApproximateTimeSynchronizer
        self.odomSub = message_filters.Subscriber(
            self, Odometry, "/odom", qos_profile=qos_profile_odom
        )
        self.laserScanSub = message_filters.Subscriber(
            self, LaserScan, "/scan", qos_profile=qos_profile_laserScanner
        )
        ...
        self.particleMarkerArrayPublisher = self.create_publisher(
            MarkerArray, "/particles/markers", 10
        )
    ...
    def initializeParticleFilter(self, x, y, th):
        ...
        self.particlePoses = np.random.uniform(
            low=[x - self.std_particle_x, y - self.std_particle_y, th - 1.0],
            high=[x + self.std_particle_x, y + self.std_particle_y, th + 1.0],
            size=(numParticles, 3),
        )

        self.particles = [Particle(p, 1 / numParticles) for p in self.particlePoses]
    ...
    def initialPose2Dcallback(self, marker: PoseWithCovarianceStamped):
        x, y, th = (
            ...
            euler_from_quaternion(marker.pose.pose.orientation),
        )
    ...
    def resample(self, laser_scan, mapUtilInstance):
        ...
        particles_weights = np.array(
            [each_particle.getWeight() for each_particle in self.particles]
        )
    ...
    def filterCallback(self, odomMsg: Odometry, laserMsg: LaserScan):
        ...
        if self.laser_to_ego_transform is None:
            ...
            try:
                ...
                self.laser_to_ego_transform = np.array(
                    [
                        [
                            np.cos(laser_to_ego_theta),
                            -np.sin(laser_to_ego_theta),
                            self.laser_to_ego_transform.transform.translation.x,
                        ],
                        [
                            np.sin(laser_to_ego_theta),
                            np.cos(laser_to_ego_theta),
                            self.laser_to_ego_transform.transform.translation.y,
                        ],
                        [0, 0, 1],
                    ]
                )
                ...
                print(
                    "Got the transform from the {} to the {}".format(
        ...
        if self.dt > 0.3:
            self.get_logger().info(
                "dt {:.3f} is too high, setting it to 0.3".format(self.dt)
            )
        dt = np.min([self.dt, 0.3])

        for i, particle_ in enumerate(self.particles):
            particle_.motion_model(v, w, dt)
            particle_.calculateParticleWeight(
                laserMsg, self.mapUtilities, self.laser_to_ego_transform
            )
        ...
        mean_angle = np.arctan2(
            np.sum(np.sin(self.particlePoses[:, 2]) * self.weights),
        ...
        publishTransform(
            self.br,
            weighted_average_pose[0],
            weighted_average_pose[1],
            weighted_average_pose[2],
            stamp,
            self.ego_odom_frame_id,
        )
        ...
        end_time = time.time()
        self.get_logger().info(
            "Time taken for the filterCallback: {:.3f} seconds".format(
                end_time - start_time
            )
        )
