...
class Particle:
    ...
    def calculateParticleWeight(
        self,
        scanOutput: LaserScan,
        mapManipulatorInstance: mapManipulator,
        laser_to_odom_transformation: np.array,
    ):
        ...
        cellPositions = mapManipulatorInstance.position_2_cell(scanInMap[:, 0:2])
        ...
        cellPositions = cellPositions[
            np.logical_and.reduce(
                (
                    cellPositions[:, 0] > 0,
                    cellPositions[:, 1] > 0,
                    cellPositions[:, 0] < lm_y,
                    cellPositions[:, 1] < lm_x,
                )
            )
        ]
    ...
    def __poseToTranslationMatrix(self):
        ...
        translation = np.array(
            [[cos(th), -sin(th), x], [sin(th), cos(th), y], [0, 0, 1]]
        )
