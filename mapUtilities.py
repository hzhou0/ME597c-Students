...
class mapManipulator(Node):
    ...
    def expand_image(self, image_array):
        ...
        expanded_image = (
            np.ones(
                (
                    image_array.shape[0] + 2 * num_pixel_to_expand,
                    image_array.shape[1] + 2 * num_pixel_to_expand,
                )
            )
            * 255
        )
        expanded_image[
            num_pixel_to_expand:-num_pixel_to_expand,
            num_pixel_to_expand:-num_pixel_to_expand,
        ] = image_array
        # shift the origin
    ...
    def position_2_cell(self, pos_array):
        ...
        h = self.height
        return np.array(
            np.floor((-origin + pos_array) / res) * np.array([1, -1])
            + np.array([0, h]),
            dtype=np.int32,
        )
