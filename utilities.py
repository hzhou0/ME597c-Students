...
def convertScanToCartesian(laserScan: LaserScan):
    ...
    valid_indices = np.where(
        (ranges != 0) & (ranges <= range_max) & (ranges >= range_min)
    )
    ...
    cartesian_points = np.column_stack(
        (valid_ranges * np.cos(angles), valid_ranges * np.sin(angles))
    )
    cartesian_points_homo = np.column_stack(
        (cartesian_points, np.ones(cartesian_points.shape[0]))
    )
